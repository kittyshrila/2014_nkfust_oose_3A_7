
# 系統分析與設計_遊戲製作計畫 #
<h1>專題小組 第七組</h1>

組員:

0124027 謝蕙如

0124103 蘇易莎<br><br>

<h3 id="content">目錄</h3>
<a href="#pr">前言</a><br>
<a href="#ch1">一、利害關係人目標表 (by謝蕙如)</a><br>
<a href="#ch2">二、事件與使用案例表 (by謝蕙如)</a><br>
<a href="#ch3">三、使用案例圖 (by蘇易莎)</a><br>
<a href="#ch4">四、個別使用案例的描述 (by謝蕙如)</a><br>
<a href="#ch5">五、個別使用案例的活動圖 (by蘇易莎)</a><br>
<a href="#ch6">六、每個使用案例的名詞與概念類別列舉表 (by謝蕙如)</a><br>
<a href="#ch7">七、每個使用案例的初步類別圖 _ 分開 (by蘇易莎)</a><br>
<a href="#ch8">八、每個使用案例的初步類別圖 _ 集中 (by蘇易莎)</a><br>
<a href="#ch9">九、整個系統的初步類別圖 _ 集中 (by蘇易莎)</a><br>
<a href="#ch10">十、每個使用案例的強韌圖 (by謝蕙如)</a><br>
<a href="#ch11">十一、使用案例主要成功情節之英文名稱事件對應 (by謝蕙如)</a><br>
<a href="#ch12">十二、每個名稱事件之合約 (by謝蕙如、蘇易莎)</a><br>
<a href="#ch13">十三、使用案例之系統循序圖 (by蘇易莎)</a><br>


<h2 id="pr">●專題題目: Pudding Crush</h2>
    ●目標：
	   ●製作出具外太空為主題的遊戲  
	   ●能隨時更新關卡
   	   ●希望遊戲不易被入侵及破壞
	   ●知道玩家的喜好並推出較符合玩家喜好的關卡

<h3>●遊戲內容介紹:</h3><br>
	1.主題：以外星人侵略地球為主題，主角Pudding需打敗Monster並救出同伴方可獲勝。<br><br>
	2.角色介紹：<br><br>
<h4>主角：Pudding</h4>
角色介紹：來自火星的外星人Pudding 想要佔領地球，卻意外和來自冥王星的Monster展開了侵略地球的戰爭！
<br><img src="https://images.plurk.com/1mOHd8OjINaxbViwc2NrF7.gif"><br><br>
<h4>角色：Monster</h4>
角色介紹：來自冥王星的外星人們Monster想要佔領地球，卻碰到來自火星的外星人Pudding也想要佔領地球！究竟誰會勝出呢？
<br><img src="https://images.plurk.com/5wG4m54CNJxA811ehvZgE1.gif"><br><br>
<h4>角色：Slipper</h4>
角色介紹：Pudding的好朋友Slipper不小心被Monster抓走了！Pudding能不能從Monster手中拯救他呢？
<br><img src="https://images.plurk.com/5kSpUvReSNcpcp7LXoock0.gif"><br><br>
<h4>角色：Pinky</h4>
角色介紹：Monster的好朋友們Pinky幫助Monster佔領地球。
<br><img src="https://images.plurk.com/1HwucBFCyMEyhFnMnFzvwT.gif"><br><br>
<h4>角色：Loki</h4>
角色介紹：Monster的首領，指揮Monster與Pudding對抗！
<br><img src="https://images.plurk.com/5bpKvB4NqPGlonej3I2hgF.gif"><br><br>
<h4>角色：Sticky</h4>
角色介紹：Monster的援軍，幫助Monster與Pudding對抗！
<br><img src="https://images.plurk.com/3oTcTY3l399RvNItw1Hpzq.jpg"><br><br>
<h4>角色：Gigi</h4>
角色介紹：指引Pudding去找出「縮小藥水」的神祕人士。
<br><img src="https://images.plurk.com/4N50Z8s5M22HJBCuLifxGg.gif"><br><br>
<h4>角色：Mega</h4>
角色介紹：居住於仙后座，擁有「縮小藥水」的神祕人士。。
<br><img src="https://images.plurk.com/i7wxs4ylu2YdSov6Hp6TA.gif"><br><br>
----------
3.關卡介紹 <br>

<<戰爭的開端>><br>
場景:外太空<br>
關卡1<br>
角色：Pudding Monster x 10<br>
勝利條件：10秒內擊敗Monster x 10<br>
關卡2<br>
角色：Pudding Monster x 20<br>
勝利條件：10秒內擊敗Monster20<br>
關卡3<br>
角色：Pudding Monster x 30<br>
勝利條件：10秒內擊敗Monster x 30<br>

<<拯救好朋友>><br>
場景:草皮<br>
描述：<br>
Pudding的好朋友Slipper被Monster抓走了，Pudding在拯救他的同時，卻也發現Monster找來了強大的同伴Pinky<br>
關卡4<br>
角色：Pudding Monster x 10 Pinky x 2<br>
勝利條件：10秒內擊敗Monster x 10和Pinky x 2<br>

<<體力的補充>><br>
場景:草皮<br>
描述：<br>
為了救出Slipper，Pudding已經流失太多力氣，正準備補充體力時，卻發現Monster把他的提神飲料藏到氣爆迷宮裡去了，如果Pudding沒有在時間內喝到提神飲料，將會活活累死在迷宮裡...<br>
關卡5<br>
角色：Pudding 提神飲料 x 1 迷宮<br>
勝利條件：20秒拿到提神飲料<br>

<<遇到陷阱>>
場景:草皮<br>
描述：<br>
補充完體力的Pudding正要離開迷宮時，卻發現有一大群Monster跟Pinky對Pudding展開攻擊<br><br>
關卡6<br>
角色：Pudding Monster x 15 Pinky x 5<br>
勝利條件：10秒內擊敗Monster x 15 Pinky x 5 <br>

<< Monster的強攻>><br>
場景:草皮<br>
描述：<br>
Monster的同伴Sticky對Pudding進行強烈攻擊<br><br>
關卡7<br>
角色：Pudding Sticky x 15 <br>
勝利條件：10秒內擊敗Monster x 15 Pinky x 5 <br>

<<預言>><br>
場景:花園<br>
描述：<br>
Pudding突然遇到一位神秘人士Gigi，預言Pudding將會面臨巨大的攻擊，並指引Pudding去仙后座尋找Mega取得"縮小藥水"<br><br>
關卡8<br>
角色：Pudding Gigi Mega x 10 <br>
勝利條件：10秒內擊敗Mega x 10 <br>

<< Monster戰略>><br>
場景:花園<br>
描述：<br>
這次Monster將派出大量的士兵包圍Pudding！這次一定可以擊敗Pudding<br><br>
關卡9<br>
角色：Pudding Monster x 30 <br>
勝利條件：10秒內躲開Monster x 30 <br>

<<魔王的挑戰>><br>
場景:花園<br>
描述：<br>
首領得知Pudding已經順利擊敗許多Monster，Loki首領決定親自帶領Monster上戰場！<br><br>
關卡10<br>
角色：Pudding Monster x 20 Loki<br>
勝利條件：Pudding擊敗 Monster x 20 Loki <br>

-the end-
<br>Pudding成功佔領地球畫面。<br>



4.遊戲介面：<br><br>
	 	(1)關卡範例：<br>
<img src="https://images.plurk.com/5YJvzrYEHUFcEsjbzjWi6d.jpg"><br><br>
		(2)過關畫面：<br>
<img src="https://images.plurk.com/3mW7rm35Cm0jftYlzjycj1.jpg"><br><br>
		(3)失敗畫面：<br>
<img src="https://images.plurk.com/1l8JmbQ8CajO4LIbiRKFyY.jpg"><br><br>
		(4)其他故事描述：<br>
<img src="https://images.plurk.com/5zebK6OrU1VU1VuSRhCZuo.jpg"><br><br>

<a href="#content">回目錄</a>

<br>
<h4 id="ch1">一、利害關係人目標表 (by謝蕙如)</h4> <br><br>
<table style="width:100%">
  <tr>
    <td>利害關係人(參與者)</td>
    <td>目標</td>     
  </tr>
  <tr>
    <td>遊戲管理者</td>
    <td>●製作出具外太空為主題的遊戲<br>
        ●能隨時更新關卡<br>
	●希望遊戲不易被入侵及破壞<br>
	●知道玩家的喜好並推出較符合玩家喜好的關卡<br>
    </td>     
  </tr>
  <tr>
    <td>顧客(玩家)</td>
    <td>●正常的遊戲流暢度<br>
        ●遊戲改版、維修的速度提升<br>
    </td>     
  </tr>
</table>
<br>
<a href="#content">回目錄</a><br>
<h4 id="ch2">二、事件與使用案例表 (by謝蕙如)</h4> <br><br>
<table style="width:100%">
  <tr>
    <td>事件名稱</td>
    <td>使用案例名稱</td>     
  </tr>
  <tr>    
    <td>1.建立遊戲(Pudding Crush)<br>
		2.能夠對遊戲進行修改<br>
		3.建立安全性檢查<br>
		4.玩家喜好分析<br>
    </td>
     <td>1.遊戲製作作業<br>
		2.遊戲更新作業<br>
		3.安全性維護作業<br>
		4.玩家喜好分析作業<br>
    </td>
   </tr>
</table>
<a href="#content">回目錄</a><br>
<br>
<h4 id="ch3">三、使用案例圖 (by蘇易莎) </h4><br><br>
<img src="https://cacoo.com/diagrams/QDswZZrTD3wrMA2d-A07B4.png">
<br><br>
<a href="#content">回目錄</a><br>
<h4 id="ch4">四、個別使用案例的描述 (by謝蕙如)</h4> <br><br>

1.「遊戲製作作業」之使用案例<br><br>
<table style="width:100%">
  <tr>
    <td>使用案例名稱</td>
    <td colspan=2>遊戲製作作業。</td>    
  </tr>
  <tr>
    <td>使用案例描述</td>
    <td colspan=2>建立遊戲內容：遊戲構想討論、背景故事、關卡設計、實際製作</td>    
  </tr>
  <tr>
    <td>主要參與者</td>
    <td colspan=2>遊戲開發者。</td>    
  </tr>
  <tr>
    <td>利害關係人與目標</td>
    <td colspan=2>遊戲開發者：將遊戲製作完整，並可實際執行。</td>    
  </tr>
  <tr>
    <td>前置條件</td>
    <td colspan=2>根據遊戲背景故事，設計出十道關卡</td>    
  </tr>
  <tr>
    <td>後置條件</td>
    <td colspan=2>利用遊戲製作軟體，實際製作出所設計的遊戲。</td>    
  </tr>
  <tr>    
    <td rowspan=2>主要成功情節    
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    </td>
    <td>參與者
    </td>
    <td>系統
    </td>
  </tr>
  <tr>
    <td>1.遊戲開發者討論遊戲背景內容<br><br>
		2.遊戲開發者討論出欲做出哪種類型的遊戲。<br><br><br>
        3.利用Multimedia Fusion2軟體製作<br><br>
		4.遊戲開發者進行大量的遊戲測試。<br><br><br>
		5.輸出遊戲並正式供玩家進行遊戲。<br><br>
    </td>
    <td>
		
		1.1以外太空為主題設計外星人佔領地球的故事。<br><br><br>
		2.1以鍵盤上下左右鍵控制遊戲主角，幫助主角攻擊或閃躲敵人。<br><br><br>
     	3.1建立遊戲物件、遊戲規則、勝利與失敗的條件以及關卡設置<br><br><br>
		4.1每一關卡的所有物件都要都要確定是否缺失，並符合遊戲開發者所設定的條件。<br><br>
		4.2選取儲存鍵，儲存玩家記錄，如<br>果按取消鍵，則回到玩家基本資料作業畫面。<br><br><br>
		5.1軟體輸出並提供載點供玩家下載。<br><br>
		


    </td>
  </tr>
  <tr>
    <td>例外情節
    </td>
    <td colspan=2>無。</td>
  </tr>
  <tr>
    <td>其他需求
    </td>
    <td colspan=2>無。
    </td>
  </tr>
</table>

<br>
2.「遊戲更新作業」之使用案例<br>

<table style="width:100%">
  <tr>
    <td>使用案例名稱</td>
    <td colspan=2>遊戲更新作業。</td>    
  </tr>
  <tr>
    <td>使用案例描述</td>
    <td colspan=2>遊戲管理者進行更新作業或是定時推出新的關卡。</td>    
  </tr>
  <tr>
    <td>主要參與者</td>
    <td colspan=2>遊戲管理者。</td>    
  </tr>
  <tr>
    <td>利害關係人與目標</td>
    <td colspan=2>遊戲管理者：讓遊戲能夠保有趣味性和完整性。</td>    
  </tr>
  <tr>
    <td>前置條件</td>
    <td colspan=2>無。</td>    
  </tr>
  <tr>
    <td>後置條件</td>
    <td colspan=2>正確修正遊戲、更新關卡。</td>    
  </tr>
  <tr>    
    <td rowspan=2>主要成功情節    
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    </td>
    <td>參與者
    </td>
    <td>系統
    </td>
  </tr>
  <tr>
    <td>1.進行遊戲更新時，遊戲管理者需公告，進行修護動作。<br><br>
		2.遊戲管理者進入遊戲更新作業，新增關卡。<br><br>
    </td>
    <td>1.1　遊戲管理者進入遊戲更新作業，修改程式。<br><br>
		2.1　遊戲管理者定期更新遊戲關卡並儲存。<br><br>


    </td>
  </tr>
  <tr>
    <td>例外情節
    </td>
    <td colspan=2>無。
    </td>
  </tr>
  <tr>
    <td>其他需求
    </td>
    <td colspan=2>無。
    </td>
  </tr>
</table>
<br>
3.「安全性維護作業」之使用案例<br>

<table style="width:100%">
  <tr>
    <td>使用案例名稱</td>
    <td colspan=2>安全性維護作業。</td>    
  </tr>
  <tr>
    <td>使用案例描述</td>
    <td colspan=2>當遊戲管理者發現某些玩家進行遊戲時，沒有透過正當程序容易過關或是遊戲發生異常，遊戲管理者適當的處理。</td>    
  </tr>
  <tr>
    <td>主要參與者</td>
    <td colspan=2>遊戲管理者。</td>    
  </tr>
  <tr>
    <td>利害關係人與目標</td>
    <td colspan=2>遊戲管理者：維護遊戲的公平性與每個玩家的權益。</td>    
  </tr>
  <tr>
    <td>前置條件</td>
    <td colspan=2>無。</td>    
  </tr>
  <tr>
    <td>後置條件</td>
    <td colspan=2>正確查出異常原因與適當的處理方式。</td>    
  </tr>
  <tr>    
    <td rowspan=2>主要成功情節    
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    </td>
    <td>參與者
    </td>
    <td>系統
    </td>
  </tr>
  <tr>
    <td>1.當玩家透過不正當的程序進行遊戲時，遊戲管理者適時的發覺並警告。<br><br>
		2.當遊戲本身發生異常時，則進入安全性維護作業。<br><br>
    </td>
    <td>1.1 遊戲管理者在系統端發現異常，找出原因所在，若為玩家透過不正當的程序進行遊戲，則警告其玩家。<br><br>
		2.1 進行安全性檢查並公告進行修護作業。<br><br>
    </td>
  </tr>
  <tr>
    <td>例外情節
    </td>
    <td colspan=2>無。
    </td>
  </tr>
  <tr>
    <td>其他需求
    </td>
    <td colspan=2>無。
    </td>
  </tr>
</table>
<br>

4.「玩家喜好分析作業」之使用案例<br>

<table style="width:100%">
  <tr>
    <td>使用案例名稱</td>
    <td colspan=2>玩家喜好分析作業。</td>    
  </tr>
  <tr>
    <td>使用案例描述</td>
    <td colspan=2>遊戲管理者對玩家進行問卷調查或是從玩家的遊戲紀錄中進行分析，並了解玩家喜好，以供遊戲參考或更新。</td>    
  </tr>
  <tr>
    <td>主要參與者</td>
    <td colspan=2>遊戲管理者。</td>    
  </tr>
  <tr>
    <td>利害關係人與目標</td>
    <td colspan=2>遊戲管理者：增加遊戲銷售量。</td>    
  </tr>
  <tr>
    <td>前置條件</td>
    <td colspan=2>無。</td>    
  </tr>
  <tr>
    <td>後置條件</td>
    <td colspan=2>Google 問卷或Excel統計分析軟體進行分析作業。</td>    
  </tr>
  <tr>    
    <td rowspan=2>主要成功情節    
    <br>
    <br>
    <br>
    <br>
    <br>
    <br>
    </td>
    <td>參與者
    </td>
    <td>系統
    </td>
  </tr>
  <tr>
    <td>1.遊戲管理者為了解玩家喜好，定期對玩家進行喜好分析。<br><br>
		2.遊戲管理者改善其遊戲內容。<br><br>
    </td>
    <td>1.1 遊戲管理者建立Google問卷表單，對玩家進行調查，並分析其結果。<br><br>
		1.2 利用遊戲紀錄的數據，了解玩家偏好，進行分析。<br><br>
		2.1 提供數據供遊戲開發者參考，使遊戲更符合玩家需求。<br><br>
    </td>
  </tr>
  <tr>
    <td>例外情節
    </td>
    <td colspan=2>無。
    </td>
  </tr>
  <tr>
    <td>其他需求
    </td>
    <td colspan=2>無。
    </td>
  </tr>
</table>
<br>
<a href="#content">回目錄</a><br>
<h4 id="ch5">五、個別使用案例的活動圖 (by蘇易莎)</h4> <br><br> 
1.「遊戲製作作業」之活動圖<br>
<img src="https://images.plurk.com/3A8Ws17qFKVHikdJiEGY4i.jpg"><br><br>
2.「遊戲更新作業」之活動圖<br>
<img src="https://images.plurk.com/4hfTdJjlgdE7yE2Ri8kxfe.jpg"><br><br>
3.「安全性維護作業」之活動圖<br>
<img src="https://images.plurk.com/OpUDJuVUmjZob0smaoVAw.jpg"><br><br>
4.「玩家喜好分析作業」之活動圖
<img src="https://images.plurk.com/16zYdzgHDj7kIFC6D0szWN.jpg"><br><br>
<br>
<a href="#content">回目錄</a><br>
<h4 id="ch6">六、每個使用案例的名詞與概念類別列舉表 (by蘇易莎)</h4> <br><br>

<table style="width:100%">
  <tr>
    <td>名詞</td>
    <td>原因</td>
	<td>結果(是否為概念類別)</td>      
  </tr>
  <tr>
    <td>玩家</td>
    <td>本遊戲需要有遊戲參與者，所以玩<br>
		家必需成為一個需要處理<br>的個體。</td> 
	<td>是</td> 
        
       
  </tr>
  <tr>
    <td>玩家基本資料</td>
    <td>本遊戲需要建立玩家基本資料</td>
	<td>是</td>

  </tr>
  <tr>
    <td>遊戲管理者</td>
    <td>本遊戲需要遊戲管理人員
    </td>
	<td>是</td>     
  </tr>
  <tr>
    <td>更新作業</td>
    <td>為遊戲管理者屬性
    </td>
	<td>否</td>     
  </tr>
  <tr>
    <td>安全性維護</td>
    <td>
		為遊戲管理者屬性
    </td> 
	<td>否</td>    
  </tr>
  <tr>
    <td>玩家喜好分析</td>
    <td>●正常的遊戲流暢度<br>
        ●加快遊戲改版、維修的速度<br>
    </td>
	<td>否</td>     
  </tr>
  <tr>
    <td>遊戲開發者</td>
    <td>●正常的遊戲流暢度<br>
        ●加快遊戲改版、維修的速度<br>
    </td>
	<td>是</td>     
  </tr>
</table>
<a href="#content">回目錄</a><br>
<h4 id="ch7">七、每個使用案例的初步類別圖_分開 (by蘇易莎)</h4> <br><br>
●boundary<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-1FC42.png"><br><br>
●countroller<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-F1578.png"><br><br>
●entity<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-42859.png"><br><br>
<a href="#content">回目錄</a><br>

<h4 id="ch8">八、每個使用案例的初步類別圖_集中 (by蘇易莎) </h4> <br><br>
●「遊戲製作作業」之初步類別圖<br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-81B3A.png"><br><br>

●「遊戲更新作業」之初步類別圖<br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-1169D.png"><br><br>

●「安全性維護作業」之初步類別圖<br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-65281.png"><br><br>

●「玩家喜好分析作業」之初步類別圖<br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-9A5E9.png"><br><br>
<a href="#content">回目錄</a><br>

<h4 id="ch9">九、整個系統之初步類別圖_集中 (by蘇易莎)</h4> <br><br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-6A2F7.png"><br><br>
<br>
<h4 id="ch10">十、每個使用案例的強韌圖 (by謝蕙如)</h4> <br><br>
●「遊戲製作作業」之強韌圖<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-6F8A0.png"><br><br>
●「遊戲更新作業」之強韌圖<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-6F8A0.png"><br><br>
●「安全性維護作業」之強韌圖<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-9C6AE.png"><br><br>
●「玩家喜好分析作業」之強韌圖<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-3A567.png"><br><br>
●「整個系統」之強韌圖<br>
<img src="https://cacoo.com/diagrams/TOlAZOqxCrCMMeQP-2F034.png"><br><br>
<a href="#content">回目錄</a><br>

<h4 id="ch11">十一、使用案例主要成功情節之英文名稱事件對應 (by謝蕙如) </h4> <br><br>    
1.「遊戲製作作業」之主要成功情節
<table style="width:100%">
  <tr>
    <td colspan=2> 使用案例：遊戲製作作業 </td> 
  </tr>
  <tr>
    <td>主要成功情節：<br><br>
		1.遊戲開發者討論出遊戲背景內容：
		以外太空為主題，設計外星人佔領地球的故事。<br><br>
		2.遊戲開發者討論出欲做出哪種類型的遊戲：用鍵盤上下左右鍵控制遊戲主角，閃躲敵人。<br><br>
		3.遊戲製作：建立物件、遊戲規則、勝利與失敗的條件、關卡設置<br><br>
		4.按下start按鈕開始遊戲，按help介紹遊戲如何進行。開始遊戲後按上下左右鍵控制主角閃躲敵人。<br><br>
		5.won的話進入下一關，lose就重複上一關。<br><br>
		6.完成遊戲。<br><br>
		7.遊戲開發者進行大量的遊戲測試，檢查是否有所缺陷。<br><br>
		8.修正遊戲。<br><br>
		9.正式提供給載點供玩家下載來玩。<br><br>

	</td>
       <td> <br><br><br><br><br><br><br><br><br>
			set(object：active,rule：active,mission：active)：active;<br><br><br>
			gamestart(start：button,help：button)：button;<br>
			actionmove(up：key,down：key,left：key,right：key)：key;<br><br>
			against(won：result,lose：result)：result;<br><br><br><br>
			test();<br><br>
			checkgame();<br><br>
			download();<br><br>
	</td>
    
  </tr>
</table>

<br>
2.「遊戲更新作業」之主要成功情節
<table style="width:100%">
  <tr>
    <td colspan=2>使用案例：遊戲更新作業 </td> 
  </tr>
  <tr>
    <td>主要成功情節：<br><br><br>
		1.遊戲管裡者準備更新遊戲時，向玩家公告。<br><br>
		2.更新遊戲：遊戲管理者進入遊戲更新作業，新增關卡。<br><br>
		3.更新完成。<br><br>

	</td>
       <td> informplayer();<br><br>
			update();
	</td>
  </tr>
</table>
3.「安全性維護作業」之主要成功情節
<table style="width:100%">
  <tr>
    <td colspan=2>使用案例：安全性維護作業 </td> 
  </tr>
  <tr>
    <td>主要成功情節：<br><br>
		1.當遊戲管理者在系統端發現異常，找出原因所在，若為玩家透過不正當的程序進行遊戲，則警告其玩家。<br><br>
		2.當遊戲本身發生異常時，則進入安全性維護作業，進行安全性檢查並公告進行修護。
	</td>
       <td> 
			<br><br>strangefound(playerfault：playerfault,systemfault：strange);<br><br>
			informplayer();<br><br>
			securitycheck();<br><br>
	</td>
  </tr>
</table>
4.「玩家喜好分析」之主要成功情節
<table style="width:100%">
  <tr>
    <td colspan=2>使用案例：玩家喜好分析 </td> 
  </tr>
  <tr>
    <td>主要成功情節：<br><br>
		1.遊戲管理者建立Google問卷表單，對玩家進行調查，或利用遊戲紀錄的數據，了解玩家偏好，進行分析。<br><br>
		2.提供數據供遊戲開發者參考，使遊戲更符合玩家喜好。
	</td>
       <td> 
			survey(googlequestionaire：survey,record：survey)：survey;<br><br>
			conclusion();
	</td>
  </tr>
</table>
<br>
<a href="#content">回目錄</a><br>

<h4 id="ch12">十二、每個名稱事件之合約 (by謝蕙如、蘇易莎) </h4> <br><br>
「遊戲製作作業」之合約  (by謝蕙如)<br>
合約1：set();
<table style="width:100%">
  <tr>
    <td colspan=2>合約1：set();合約</td> 
  </tr>
  <tr>
	 <td>操作：set(object：active,rule：active,mission：active)：active;<br>
		交互參照：遊戲製作作業<br>
		前置條件：規劃好遊戲主題、物件、規則、關卡<br>
		後置條件：實際用軟體製作出來<br>
		</td>
</tr>
</table>

合約2：gamestart();
<table style="width:100%">
  <tr>
    <td colspan=2>合約2：gamestart(); </td> 
  </tr>
  <tr>
	 <td>操作：gamestart(start：button,help：button)：button;<br>
		交互參照：遊戲製作作業<br>
		前置條件：存在set之實例object,rule,mission<br>
		後置條件：-設定出gamestart(start：button,help：button)：button;之條件。<br>
		</td>
</tr>
</table>
合約3：actionmove();
<table style="width:100%">
  <tr>
    <td colspan=2>合約1：actionmove(); </td> 
  </tr>
  <tr>
	 <td>操作：actionmove(up：key,down：key,left：key,right：key)：key;<br>
		交互參照：遊戲製作作業<br>
		前置條件：存在set之實例rule<br>
		後置條件：依照所設定的rule，執行actionmove(up：key,down：key,left：key,right：key)：key;<br>
		</td>
</tr>
</table>
合約4：against();
<table style="width:100%">
  <tr>
    <td colspan=2>合約4：against(); </td> 
  </tr>
  <tr>
	 <td>操作：against(won：result,lose：result)：result;<br>
		交互參照：遊戲製作作業<br>
		前置條件：存在set之實例rule<br>
		後置條件：依照所設定的rule，執行against(won：result,lose：result)：result;<br>
		</td>
</tr>
</table>
合約5：test();
<table style="width:100%">
  <tr>
    <td colspan=2>合約5：test(); </td> 
  </tr>
  <tr>
	 <td>操作：test()<br>
		交互參照：遊戲製作作業<br>
		前置條件：依據set(object：active,rule：active,mission：active);、gamestart(start：button,help：button)：button;、actionmove(up：key,down：key,left：key,right：key)：key;、against(won：result,losewon：result)won：result;來進行test();<br>
		後置條件：執行test();,並確認是否有無錯誤環節。<br>
		</td>
</tr>
</table>
合約6：checkgame();
<table style="width:100%">
  <tr>
    <td colspan=2>合約6：checkgame(); </td> 
  </tr>
  <tr>
	 <td>操作：checkgame();<br>
		交互參照：遊戲製作作業<br>
		前置條件：存在set之實例<br>
		後置條件：將test();中發現的錯誤環節，進行checkgame();。<br>
		</td>
</tr>
</table>
合約7：download();
<table style="width:100%">
  <tr>
    <td colspan=2>合約7：download(); </td> 
  </tr>
  <tr>
	 <td>操作：download();<br>
		交互參照：遊戲製作作業<br>
		前置條件：將遊戲製作完並修正完整。<br>
		後置條件：正式提供玩家download();<br>
		</td>
</tr>
</table>
「遊戲更新作業」之合約  (by謝蕙如)<br>
合約1：imformplayer();
<table style="width:100%">
  <tr>
    <td colspan=2>合約1：imformplayer(); </td> 
  </tr>
  <tr>
	 <td>操作：imformplayer();<br>
		交互參照：遊戲更新作業<br>
		前置條件：在update();之前先imformplayer();<br>
		後置條件：imformplayer();後再進行update();<br>
		</td>
</tr>
</table>
合約2：update();
<table style="width:100%">
  <tr>
    <td colspan=2>合約2：iupdate(); </td> 
  </tr>
  <tr>
	 <td>操作：imformplayer();<br>
		交互參照：遊戲更新作業<br>
		前置條件：存在informplayer();之實例<br>
		後置條件：進行update();<br>
		</td>
</tr>
</table>
「安全性維護作業」之合約  (by蘇易莎)<br>
</table>
合約1：strangefound();
<table style="width:100%">
  <tr>
    <td colspan=2>合約1：strangefound(); </td> 
  </tr>
  <tr>
	 <td>操作：strangefound(playerfault：strange,systemfault：strange)：strange;<br>
		交互參照：安全性維護作業<br>
		前置條件：在系統端發現strangefound(playerfault：strange,systemfault：strange)：strange;<br>
		後置條件：-若為玩家所為則informplayer();,若是系統本身的問題則securitycheck();<br>
		</td>
</tr>
</table>
合約2：informplayer();
<table style="width:100%">
  <tr>
    <td colspan=2>合約2：informplayer(); </td> 
  </tr>
  <tr>
	 <td>操作：informplayer();<br>
		交互參照：安全性維護作業<br>
		前置條件：在系統端發現strangefound(playerfault：strange,systemfault：strange)：strange;，並確認為玩家所為。<br>
		後置條件：執行informplayer();<br>
		</td>
</tr>
</table>
合約3：securitycheck();
<table style="width:100%">
  <tr>
    <td colspan=2>合約3：securitycheck(); </td> 
  </tr>
  <tr>
	 <td>操作：securitycheck();<br>
		交互參照：安全性維護作業<br>
		前置條件：在系統端發現strangefound(playerfault：strange,systemfault：strange)：strange;，並確認為系統所為。<br>
		後置條件：執行securitycheck();<br>
		</td>
</tr>
</table>
「玩家喜好分析」之合約  (by蘇易莎)<br>
合約1：survey();
<table style="width:100%">
  <tr>
    <td colspan=2>合約1：survey();</td> 
  </tr>
  <tr>
	 <td>操作：survey(googlequestionaire：survey,record：survey)：survey;<br>
		交互參照：安全性維護作業<br>
		前置條件：遊戲管理者發放googlequestionaire或參考玩家record<br>
		後置條件：進行分析survey(googlequestionaire：survey,record：survey)：survey;<br>
		</td>
</tr>
</table>
合約2：conclusion()
<table style="width:100%">
  <tr>
    <td colspan=2>合約2：conclusion();</td> 
  </tr>
  <tr>
	 <td>操作：conclusion();<br>
		交互參照：安全性維護作業<br>
		前置條件：存在survey之實例googlequestionaire、record<br>
		後置條件：依據survey(googlequestionaire：survey,record：survey)：survey;進行conclusion();。<br>
		</td>
</tr>
</table>
<a href="#content">回目錄</a><br>

<h4 id="ch13">十三、使用案例之循序圖 (by蘇易莎)</h4> <br><br>
1.「遊戲製作作業」之循序圖<br><br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-D2D96.png">
<br><br>
2.「遊戲更新作業」之循序圖<br><br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-538E9.png">
<br><br>
3.「安全性維護作業」之循序圖<br><br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-3AFB8.png">
<br><br>
4.「玩家喜好分析」之循序圖<br><br>
<img src="https://cacoo.com/diagrams/NXozh7qchzytFNMx-7FF5B.png">
<br><br>
5.「Pudding Crash」之系統循序圖<br><br>
<img src="https://cacoo.com/diagrams/apGodGLXXvsrnPLs-EBF99.png">
<br><br>
<img src="https://cacoo.com/diagrams/apGodGLXXvsrnPLs-E70A4.png">
<br><br>
<img src="https://cacoo.com/diagrams/9FKIDo72z9M0itJX-2250A.png">
<br><br>
<a href="#content">回目錄</a><br>


	




























